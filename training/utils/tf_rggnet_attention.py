...
def rggnet_forward(x_dm_ft, x_cam, is_training):
    ...
    with tf.name_scope('Forward'):
        with tf.name_scope('Feature_Extraction'):
            ...
            # add attention module _ Moon
            f_dm_attention = cbam_block(f_dm, 'cbam_depth_layer1', ratio=reduction_ratio)
            f_cam_attention = cbam_block(f_cam, 'cbam_cam_layer1', ratio=reduction_ratio)
        with tf.variable_scope('Fuse'):
            feature = tf.concat([f_dm_attention, f_cam_attention], -1)
